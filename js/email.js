...
DA.email = (function () {
  ...
  const COLOR_MAP = {
    blue: { bg: 'bg-blue-100', text: 'text-blue-600', groupHover: 'group-hover:bg-indigo-600', ring: 'group-hover:border-indigo-500', shadow: 'hover:shadow-indigo-500/10', accent: 'text-indigo-600' },
    emerald: { bg: 'bg-emerald-100', text: 'text-emerald-600', groupHover: 'group-hover:bg-emerald-600', ring: 'group-hover:border-emerald-500', shadow: 'hover:shadow-emerald-500/10', accent: 'text-emerald-600' },
    purple: { bg: 'bg-purple-100', text: 'text-purple-600', groupHover: 'group-hover:bg-purple-600', ring: 'group-hover:border-purple-500', shadow: 'hover:shadow-purple-500/10', accent: 'text-purple-600' },
    slate: { bg: 'bg-slate-100', text: 'text-slate-600', groupHover: 'group-hover:bg-slate-700', ring: 'group-hover:border-slate-500', shadow: 'hover:shadow-slate-500/10', accent: 'text-slate-700' },
  };

  function cardEl(t, isSaved, index) {
    const c = COLOR_MAP[t.color] || COLOR_MAP.blue;
    const div = document.createElement('div');
    div.className = `group bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 cursor-pointer shadow-sm hover:shadow-xl ${c.shadow} ${c.ring} hover:-translate-y-1 transition-all duration-300 relative overflow-hidden`;
    div.innerHTML = `
      <div class="relative z-10">
        <div class="w-10 h-10 ${c.bg} ${c.text} rounded-lg flex items-center justify-center mb-4 ${c.groupHover} group-hover:text-white transition-colors">
          <i class="fa-solid ${t.icon}"></i>
        </div>
        <h3 class="font-bold text-slate-800 dark:text-slate-100 mb-2 text-lg">${escapeHtml(t.name)}</h3>
        <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed mb-4">${escapeHtml(t.desc || 'Template kustom Anda')}</p>
        <span class="text-xs font-semibold ${c.accent} group-hover:underline">
          Gunakan Template <i class="fa-solid fa-arrow-right ml-1"></i>
        </span>
      </div>
      ${isSaved ? `<button class="absolute top-3 right-3 w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-red-500 hover:bg-red-50 transition-colors z-20" data-del><i class="fa-solid fa-trash-can text-xs"></i></button>` : ''}
    `;
